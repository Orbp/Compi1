...
public class CrearArchivo extends javax.swing.JFrame {
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        if(this.jTextField1.getText().isEmpty() || this.jTextField2.getText().isEmpty()){
            JOptionPane.showMessageDialog(this, "No se puede dejar ninguno de los campos en blanco", "ERROR", JOptionPane.ERROR_MESSAGE);
        }else{
            path = jTextField1.getText();
            nombre = jTextField2.getText() + ".cp";
            File archivo = new File(path + "\\" + nombre);
            if(archivo.exists()){
                JOptionPane.showMessageDialog(this, "El archivo que se quiere crear ya existe debe de cambiar el nombre", "ERROR", JOptionPane.ERROR_MESSAGE);
            }else{
                if(!VerificarTabAbierta(path + "\\" + nombre)){
                    tab tabb = new tab();
                    tabb.setNombre(nombre);
                    tabb.setPath(path);
                    ventana.jTabbedPane1.add(nombre, tabb);
                    File archivo1 = new File(path + "\\" + nombre);
                    ventana.jTabbedPane1.setSelectedIndex(ventana.jTabbedPane1.getTabCount() - 1);
                    try {
    ...
    public boolean VerificarTabAbierta(String path){
        int numpesta = ventana.jTabbedPane1.getTabCount();
        for(int i = 0; i < numpesta; i++){
            tab tabb = (tab)ventana.jTabbedPane1.getComponent(i);
            String aux = tabb.getPath() + "\\" + tabb.getNombre();
            if(aux.equals(path)){
                ventana.jTabbedPane1.setSelectedIndex(i);
                return true;
            }
        }
        return false;
